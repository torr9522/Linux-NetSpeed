...
show_kernel_install_finish_notice() {
  ...
  echo -e "${Tip} 重启VPS后，请重新运行脚本开启${Red_font_prefix}${feature}${Font_color_suffix}"
}
...
installbbr() {
  ...
  detele_kernel
  BBR_grub
  show_kernel_install_finish_notice "BBR"
  check_kernel
...
installbbrplus() {
  ...
  if [[ "${OS_type}" == "CentOS" ]]; then
    if [[ ${version} == "7" ]]; then
      if [[ ${bit} == "x86_64" ]]; then
        ...
        detele_kernel_head
        headurl=https://raw.githubusercontent.com/torr9522/Linux-NetSpeed/tcp.sh/bbrplus/centos/7/kernel-headers-4.14.129-bbrplus.rpm
        imgurl=https://raw.githubusercontent.com/torr9522/Linux-NetSpeed/tcp.sh/bbrplus/centos/7/kernel-4.14.129-bbrplus.rpm
    ...
    if [[ ${bit} == "x86_64" ]]; then
      ...
      detele_kernel_head
      headurl=https://raw.githubusercontent.com/torr9522/Linux-NetSpeed/tcp.sh/bbrplus/debian-ubuntu/x64/linux-headers-4.14.129-bbrplus.deb
      imgurl=https://raw.githubusercontent.com/torr9522/Linux-NetSpeed/tcp.sh/bbrplus/debian-ubuntu/x64/linux-image-4.14.129-bbrplus.deb
  ...
  detele_kernel
  BBR_grub
  show_kernel_install_finish_notice "BBRplus"
  check_kernel
...
installlot() {
  ...
  detele_kernel
  BBR_grub
  show_kernel_install_finish_notice "Lotserver"
  check_kernel
...
startlotserver() {
  ...
  echo | bash <(wget --no-check-certificate -qO- https://raw.githubusercontent.com/torr9522/Linux-NetSpeed/tcp.sh/selfhost/lotServerInstall.sh) install
...
remove_bbr_lotserver() {
  ...
  if [[ -e /appex/bin/lotServer.sh ]]; then
    echo | bash <(wget -qO- https://raw.githubusercontent.com/torr9522/Linux-NetSpeed/tcp.sh/selfhost/lotServerInstall.sh) uninstall
  fi
  clear
...
remove_all() {
  ...
  sed -i '/net.ipv4.tcp_max_orphans/d' /etc/sysctl.conf
  if [[ -e /appex/bin/lotServer.sh ]]; then
    bash <(wget -qO- https://raw.githubusercontent.com/torr9522/Linux-NetSpeed/tcp.sh/selfhost/lotServerInstall.sh) uninstall
  fi
  clear
  echo -e "${Info}:清除加速完成。"
  sleep 1s
...
Update_Shell() {
  ...
  local shell_url="https://raw.githubusercontent.com/torr9522/Linux-NetSpeed/tcp.sh/tcp.sh"
...
gototeddysun_bbr() {
  ...
  bash <(wget -qO- https://raw.githubusercontent.com/torr9522/Linux-NetSpeed/tcp.sh/selfhost/bbr.sh)
}
...
gotodd() {
  ...
  bash <(wget -qO- https://raw.githubusercontent.com/torr9522/Linux-NetSpeed/tcp.sh/selfhost/NewReinstall.sh)
...
gotoipcheck() {
  clear
  sleep 1.5
  bash <(wget -qO- https://raw.githubusercontent.com/torr9522/Linux-NetSpeed/tcp.sh/selfhost/ip.sh)
...
————————————————————————————————————————————————————————————————" &&
  ...
  echo -e " 拥塞控制算法:: ${Green_font_prefix}${net_congestion_control}${Font_color_suffix} 队列算法: ${Green_font_prefix}${net_qdisc}${Font_color_suffix} "
